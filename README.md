# Money Transfer Service

REST API для перевода денег с карты на карту.

## Функциональность

- Перевод денег между банковскими картами
- Валидация данных карт
- Логирование операций в файл
- Поддержка CORS для фронтенда

## Запуск приложения

```bash
./gradlew bootRun
```
## Docker запуск
### Запуск через Docker Compose:
```bash
docker-compose up
```
## Сборка и запуск образа

```bash
docker build -t money-transfer-service:1.0 .
docker run -p 8080:8080 money-transfer-service:1.0
```
## 🧪 Тестирование

### Юнит-тесты (Mockito)
- `TransferControllerTest` - тестирование REST контроллера
- `TransferServiceTest` - тестирование бизнес-логики перевода
- `LoggingServiceTest` - тестирование сервиса логирования

### Интеграционные тесты Testcontainers

- `MoneyTransferIntegrationTest` - интеграционный тест приложения в Docker:
    - Запускает собранный Docker образ `money-transfer-service:1.0`
    - Проверяет доступность health endpoint (`/health`)
    - Тестирует работу Spring Boot приложения в изолированной среде
    - Использует реальный порт 8080 внутри контейнера

**Запуск всех тестов:**
```bash
./gradlew test
```
## Интеграция с FRONT
### API настроено на порт 8080 для совместимости с FRONT приложением из репозитория:
https://github.com/serp-ya/card-transfer

### Тестирование API через Postman:
```
POST http://localhost:8080/transfer
Content-Type: application/json

{
"cardFromNumber": "1234567812345678",
"cardFromValidTill": "12/25",
"cardFromCVV": "123",
"cardToNumber": "8765432187654321",
"amount": {
"value": 10000,
"currency": "RUB"
}
}
```
## API Endpoints

### POST /transfer
Выполняет перевод денег между картами.

Пример запроса:
```json
{
  "cardFromNumber": "1234567812345678",
  "cardFromValidTill": "12/25",
  "cardFromCVV": "123",
  "cardToNumber": "8765432187654321",
  "amount": {
    "value": 10000,
    "currency": "RUB"
  }
}
```
Пример ответа:
```
{
  "operationId": "a1b2c3d4-e5f6-7890-abcd-ef1234567890"
}
```
### POST /confirmOperation
Подтверждение операции с кодом.

Пример запроса:
```json
{
  "operationId": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
  "code": "0000"
}
```
Пример ответа (200 OK):
```json
{
"operationId": "a1b2c3d4-e5f6-7890-abcd-ef1234567890"
}
```
Ошибки (400 Bad Request):

```json
{
"message": "Cannot transfer to the same card",
"id": 4001
}
```
### GET /health
Проверка работоспособности сервиса.
## Технологии
- Java 17
- Spring Boot 4.0.0
- Gradle
- Docker
## Архитектура приложения

```
┌─────────────┐    REST API    ┌──────────────┐    File System   ┌─────────────┐
│   FRONT     │ ←───────────→  │ REST SERVICE │ ←──────────────→ │  LOG FILE   │
│  (React)    │                │ (Spring Boot)│                  │             │
└─────────────┘                └──────────────┘                  └─────────────┘
```

## Формат хранения данных

### Данные карт:
Карты представлены в виде DTO объектов в оперативной памяти:
- **Номер карты**: 16 цифр, строковый формат
- **Срок действия**: MM/YY, валидация месяца/года
- **CVV**: 3 цифры, строковый формат
- **Сумма**: в копейках, целое число (10000 = 100.00 ₽)
- **Валюта**: RUB (фиксированная)

### Логирование операций:
Операции записываются в файл `transfer.log` в формате:
```
ГГГГ-ММ-ДД ЧЧ:ММ:СС | From: НОМЕР_КАРТЫ | To: НОМЕР_КАРТЫ | Amount: СУММА ВАЛЮТА | Result: РЕЗУЛЬТАТ | OperationID: UUID
```

## Настройки приложения
Настройки хранятся в `application.yaml`:
- Порт сервера: 8080
- Имя приложения: money-transfer-service
- Файл логов: transfer.log